...
counts must agree. Both paths use one shared acceleration-window collector, so
parity is exact rather than tolerance-based: per-scene raw metre sum, sample
count, and triple count must be bit-equal before display conversion. This is a
compatibility invariant and an implementation smoke gate.
...
src/hjlib_evaluation/corrected_crowd_protocol.py
    legacy-owned shared exact acceleration-window collection
```
...
order. The shared acceleration-window collector remains owned by the existing
legacy `corrected_crowd_protocol.py`; the additive module imports it in one
direction. Removing the additive profile therefore cannot remove a legacy
dependency.
...
performs an explicit matched-row join. One shared acceleration-window collector
is used by legacy `ACCEL-WORLD` and the new result, preserving its exact row and
summation order. A sibling jerk traversal computes the two jerk populations and
exact quadruple count. Generic finite-difference math remains method-neutral.
...
Data-free stable-owner tests must cover:
...
8. JSON round trip rejects changed schema, metric order, unit order, shapes,
   non-finite values, negative values, identity drift, numeric mask coercion,
   selected non-visible rows, and count/window invariant drift.
9. A TPA data-free selected-view golden constructs a fixed 15-metric result,
   ...
   after extracting the shared acceleration collector.
...
- 2026-08-19: Mathematical Architecture review found no Critical and four
  ...
  shared-collector bit-exact legacy parity, raw summary units/count invariants,
...
- 2026-08-19: Code Architecture review found no Critical and three Concerns.
  ...
  shared acceleration collector back under the legacy protocol owner and froze
...
- 2026-08-19: Final focused Code Architecture re-review reported zero remaining
  ...
  generic jerk leaf, legacy-owned shared acceleration traversal, GroupRec v2
