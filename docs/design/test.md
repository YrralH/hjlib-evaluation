...
端到端验证。roots 经 `test/local_setting_test.py`(gitignored)注入;缺则 import 期
AssertionError(FAIL-not-skip,不 skip)。
...
| `local_setting_test.py` | per-machine roots(gitignored):`PATH_DUMP_ROOT_HJLIB` / `PATH_FILTER_STATS_BASE` / `PATH_JTA[_EXT]_DATA_ROOT` / `PATH_INFERENCE_DUMPS_BASE` |
