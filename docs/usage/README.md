...
monolith)、对齐 GT、按 `Eval_Meta` 算 MPJPE / T-MPJPE / Jitter。**dump 必须覆盖
testset 的每一段**(否则报缺失);用 `list_dump_segment_tags(dump_dir)` 核对覆盖。
...
| `eval_dumps_against_gt(testset, gt_provider, path_dump_dir, path_pkl_for_segment)` | 无 Tester 直接归约(stage_eval 委托给它) |
